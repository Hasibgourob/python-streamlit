            ...
            if st.button("Run Query"):
                try:
                    ...
                    if query_choice == "Supplier Product Availability":
                        ...
                        st.write("Supplier Product Availability Results:")
                        st.dataframe(result)
                        st.download_button(
                            label="Download CSV",
                            data=result.to_csv(index=False),
                            file_name="supplier_product_availability.csv",
                            mime="text/csv"
                        )

                    elif query_choice == "Total Sales":
                        result = pd.read_sql_query(q2, conn)
                        st.write("Total Sales Results:")
                        st.dataframe(result)
                        st.download_button(
                            label="Download CSV",
                            data=result.to_csv(index=False),
                            file_name="total_sales.csv",
                            mime="text/csv"
                        )

                    elif query_choice == "Seller Payment Method":
                        # Fetch the payment methods from the database for suppliers in the CSV
                        ...
                        st.write("Seller Payment Method Results:")
                        st.dataframe(result)
                        st.download_button(
                            label="Download CSV",
                            data=result.to_csv(index=False),
                            file_name="seller_payment_method.csv",
                            mime="text/csv"
                        )
